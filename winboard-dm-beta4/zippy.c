...
extern int currentMove; /* from backend.c */
...
void ZippyInit()
{
	...
	/* What password is used for GUI commands? */
    p = getenv("ZIPPYPASSWORD3");
    if (p != NULL) {
      appData.zippyPassword3 = p;
    }
...
#include "trivia.c"
	...
	player = StripHighlightAndTitle(star_match[0]);
	if (appData.zippyBughouse && first.initDone &&
    ...
    if (looking_at(buf, i, "* spoofs you:")) {
        ...
        SendToICS(reply);
	  sprintf(reply, "tell %s Yes sir, i do it \n", player);
	  SendToICS(reply);
    }
    return FALSE;
}
	 ...
	 char *player;
{
    ...
    if (looking_at(buf, i, "* * match * * requested with * (*)")) {
		...
		/* don't accept matches if ICS analyze run */
		if (gameMode == IcsObserving || gameMode == IcsExamining) {	
				sprintf(reply, "tell %s Sorry, i'm analysis a game and don't accept matches. Please try it later again \n", player);
				SendToICS(reply);
				return FALSE;
		}
     ...
     int moveNum, basetime, increment;
{
	...
	/* reset ZippyDraw for a new or resumed game */
	if (appData.zippyDraw) ZippyDraw(2, 0, 0);
	...
	if (!strcmp(opp_name[w], opp)) {
		...
		/* If enable send engine output to ics we set to kibitz */
		if (appData.icsActive && appData.AnalysisWindow) appData.SendOutPutToICS = 2;
		break;
	}
    }
	...
	if (request == 0) {
		if (score == drawFactor || (score < 0 && score > -10)) {
			if (depth > 1) drawCounter++;
				if (drawCounter >= 5 && (currentMove - lastDrawMove) > 20 &&
					currentMove > 40 && depth > 1) {
					/* rating check */
					if (RatingDiff > 50) {
							sprintf(str, "whisper Winboard-DM: ZippyDraw: No draw offer - Rating of opponent to low \n");
							SendToICS(str);
							drawCounter = 0;
							return;
					}
					if (appData.debugMode) fprintf(debugFP, 
						"Offer draw. counter: %d\n", drawCounter);
						sprintf(str, "kibitz Winboard-DM: ZippyDraw = counter %d; LastDraw: %d; \n",
									drawCounter, lastDrawMove);
						SendToICS(str);
						sprintf(str, "draw \n");
						SendToICS(str);
						lastDrawMove = currentMove;
				}
		} else {
			drawCounter = 0;
		}
	} else if (request == 1) {
		if (drawCounter >= 5 && currentMove > 40 &&  depth >= 1 && score <= drawFactor && 
			RatingDiff < 50) {
			if (appData.debugMode) fprintf(debugFP, "accept draw \n");
				sprintf(str, "kibitz Winboard-DM: ZippyDraw = counter %d; LastDraw: %d; score %d R-Diff %d \n",
					  drawCounter, lastDrawMove, score, RatingDiff);
			SendToICS(str);
			sprintf(str, "draw\n");
			SendToICS(str);
			return;
		...
		if (score < -50 && depth > 1) {
					if (appData.debugMode) fprintf(debugFP, "draw \n");		
					sprintf(str, "kibitz Winboard-DM: ZippyDraw = counter %d; LastDraw: %d; score %d R-Diff %d \n",
					        drawCounter, lastDrawMove, score, RatingDiff);
					SendToICS(str);
					sprintf(str, "draw\n");
			...
			if (appData.debugMode) fprintf(debugFP, "decline Draw \n");
				sprintf(str, "kibitz Winboard-DM: ZippyDraw = counter %d; LastDraw: %d; R-Diff %d\n",
					  drawCounter, lastDrawMove, RatingDiff);

				SendToICS(str);
				sprintf(str, "kibitz No thank you. I wanna play \n");
				SendToICS(str);
		}
	} else {
		...
		if (gameMode == IcsPlayingBlack) {
			RatingDiff = gameInfo.blackRating - gameInfo.whiteRating;
		} else if (gameMode == IcsPlayingWhite) {
			RatingDiff = gameInfo.whiteRating - gameInfo.blackRating;
		}
		sprintf(str, "whisper Winboard-DM: ZippyDraw = enable dynamic draw; R-Diff %d \n", RatingDiff);
		SendToICS(str);
	}	
}
